...
def get_previous_failed_recipients_for_message(message_hash: str):
    rows = db_execute(
        "SELECT DISTINCT br.user_id FROM broadcast_recipients br JOIN broadcast_runs r ON br.run_id = r.run_id WHERE r.message_hash = ? AND br.status = 'failed'",
        (message_hash,),
        fetch=True,
    )
    return set(r[0] for r in rows) if rows else set()
...
def handle_command(user_id: int, username: str, command: str, args: str):
    ...
    if command == "/broadcast":
        ...
        # Find recipients who previously failed for this message - we will not retry them
        previously_failed = get_previous_failed_recipients_for_message(message_hash)
